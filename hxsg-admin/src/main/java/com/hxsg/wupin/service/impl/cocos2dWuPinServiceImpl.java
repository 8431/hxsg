...
public class cocos2dWuPinServiceImpl implements cocos2dWuPinService {
    ...
    public List<ZaWuDetailVo> queryWuPinByBaoShi(Integer roleid, String rolename) {
        ...
        try {
            ...
            li = getZaWuDetailVo(roleid, rolename, libs);
    ...
    private List<ZaWuDetailVo> getZaWuDetailVo(Integer roleid, String rolename, List<BaoShi> libs) {
        List<ZaWuDetailVo> li = new ArrayList<>();
        for (BaoShi b : libs) {
    ...
    private List<ZaWuDetailVo> getZaWuDetailVoForMap(Integer roleid, String rolename, List<Map<String,Object>> libs) {
        List<ZaWuDetailVo> li = new ArrayList<>();
        for (Map<String,Object> b : libs) {
            ZaWuDetailVo zo = new ZaWuDetailVo();
            String level = "0";
            zo.setId(Integer.parseInt(((Long)b.get("id")).toString()));
            zo.setRoleId(roleid);
            zo.setRoleName(rolename);
            zo.setZwMiaoShu((String)b.get("miaoShuId"));
            zo.setZwXiaoGuo(b.get("kangXing") + "+" + b.get("xiaoGuo"));
            zo.setZwName((String)b.get("name"));
            zo.setZwUrl((String)b.get("img"));
            if (Integer.parseInt(((Long) b.get("xiaoGuo")).toString()) > 15) {
                Integer a = Integer.parseInt(((Long) b.get("xiaoGuo")).toString()) / 100;
                level = a.toString();
            } else {
                level = ((Long) b.get("xiaoGuo")).toString();
            }
            zo.setZwLevel(level);
            li.add(zo);
        }
    ...
    public ZhuangBeiDetailVo queryZhuangBeiDetail(ZhuangBeiDetailVo zb, String roleName) throws Exception {
        ...
        try {
            ...
            if (rm != null) {
                ...
                zbDetail.setLevel(rm.getLevel().toString());
...
//                }
    ...
    public String mosaicGemtoZhuangBei(Integer roleid, Integer bsId, Integer zbId) throws Exception {
        ...
        try {
            ...
            throw new CommonException("mosaicGemtoZhuangBei", e.getMessage(), e);
        }
        return re;
    }
...
/*********************************************************************************************/
    ...
    @Override
    public List<ZaWuDetailVo> queryXqBaoShi(Integer roleid, String rolename, String zbid) throws CommonException {
        List<ZaWuDetailVo> li = null;
        try {
            if (StringUtils.isEmpty(roleid) || StringUtils.isEmpty(rolename) || StringUtils.isEmpty(zbid)) {
                throw new CommonException("roleid,rolename,zbid不能为空");
            }
            Map<String, Object> param = new HashMap<>();

            String sql = "select * from baoshi where roleid=#{roleid} and status='0' and find_in_set((select right(wuqitype, 1) from role_zhuangbei_ms where id=#{zbid} ) , zbtype);";
            param.put("sql", sql);
            param.put("roleid", roleid);
            param.put("zbid", zbid);
            List<Map<String, Object>> mapLi = hxsgbasedaomapper.QuerySql(param);
            li = getZaWuDetailVoForMap(roleid, rolename, mapLi);
        } catch (Exception e) {
            throw new CommonException("queryXqBaoShi", e.getMessage(), e);
        }
        return li;
    }

    @Override
    public List<Map<String, Object>> queryMyZhuangBei(String roleid) throws CommonException {
        ...
        try {
            ...
            String sql = "select wuqiType,imgUrl,name,id from role_zhuangbei_ms  where roleid=#{roleid} and zb=1";
            param.put("sql", sql);
            param.put("roleid", roleid);
