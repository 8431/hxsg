    ...
    public String roleStakeResult(Integer roleId, ChibiYazhuDetail cd) throws Exception {
        ...
        try {
            ...
            if (cd.getRoleid() != null) {
                if (cd.getYin() == null) {
                    cd.setYin(0);
                } else if (cd.getYin() > 1000000) {
                    return StatusNum.CCXZ;
                }
                if (cd.getJin() == null) {
                    cd.setJin(0);
                } else if (cd.getJin() > 10000) {
                    return StatusNum.CCXZ;
                }
                ...
                if (!StringUtils.isEmpty(cd.getResult()) && CHIBIRESULT.indexOf(cd.getResult()) != -1) {
                    ...
                    if (role != null) {
                        ...
                        if ((cd.getJin() > 0 && roleJin >= cd.getJin()) || (cd.getYin() > 0 && roleYin >= cd.getYin())) {
                            ...
                            cd.setStatus("0");
                            cd.setNum(num);
                            cd.setRolename(role.getRolename());
                ...
                if (!StringUtils.isEmpty(result) && cbi != null) {
                    ...
                    if (sumYazhu != null && sumYazhu.size() > 0) {
                        for (ChibiYazhuDetail d : sumYazhu) {
                            ...
                            //更新压住详情表
                            List<ChibiYazhuDetail> yaLi = chibiyazhudetailmapper.queryYzDetail(num, d.getRoleid());
                            for (ChibiYazhuDetail m : yaLi) {
                                if (result.equals(m.getResult())) {
                                    m.setJieguo("赢");
                                } else {
                                    m.setJieguo("输");
                                }
                                chibiyazhudetailmapper.updateByPrimaryKeySelective(m);
                            }
