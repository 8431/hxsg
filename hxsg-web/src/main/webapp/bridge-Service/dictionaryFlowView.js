$(function(){
    ...
    var js_retry = $("#js_retry");                                      // 全部重试
    var js_success = $(".js_success");                                  // 成功
    var js_fail = $(".js_fail");                                        // 失败

    // 关闭全部重试弹窗
    $(document).delegate(".knowBtn", "click", function(event){
        event.stopPropagation;
        closePop(".popUpRetry", ".iframe_box");
    });
    $(document).delegate(".retryClose", "click", function(event){
        event.stopPropagation;
        closePop(".popUpRetry", ".iframe_box");
    });
    ...
    var page_cur = 1;
    var page_total = "";
    var searchVal = [["REC_TIME>",todayTimeInit],["REC_TIME<",todayTimeEnd],["PAY_LOAD_TYPE like","%%"]];
    ...
    function flowleft(sendDate){
        ...
        $.ajax({
            ...
            success: function(json){
                ...
                var failval = json.fail;
                var succesval = json.succes;
                ...
                if(arrays != ''){
                    if(page_cur <= page_total){
                        ...
                        // 成功和失败注入
                        js_success.text(succesval);
                        js_fail.text(failval);
                        if(failval == 0){
                            $(".bgRetry").addClass("successRetry");
                        }else{
                            $(".bgRetry").addClass("failRetry");
                        };
                        ...
                        js_table_static.html("");
                        js_table_static.append('<tr>' + '<td colspan="9" class="overPage_L">' + '<p>' + "您输入的页码已超出当前内容总页码！" + '</p>' + '</td>' + '</tr>');
                        js_flowChart.html("");
                    }
                }else{
                    js_table_static.html("");
                    js_table_static.append('<tr>' + '<td colspan="9" class="overPage_L">' + '<p>' + "未能查询到相关内容！" + '</p>' + '</td>' + '</tr>');
                    js_flowChart.html("");
                    js_pagelist.hide();
                };
            }
                ...
                if(push != ''){
                    ...
                    for(var j=0;j<pushSon;j++){
                        if(j == 0){                                                                     // 解决推送队列中子队列第一个错了 2像素的问题
                            addHtmlPush =  '<div class="lineSite85" style="width:87px;left:403px;top:'+ ((100 + 100*j) + 31)+"px" +';">' + '</div>' +
                                        '<div class="arrowImg" style="left:490px;top:'+ ((100 + 100*j) + 26)+"px" +';">' + '</div>' +
                                        '<div class="title onTxt" style="top:'+ (100*(j+1)-2) +"px" +';left:410px;">' + '<p>' + push[j].ID_NAME + '</p>' + '</div>' +
    ...
    // 全部重试
    js_retry.off("click").on("click",function(){
        var sendPushAll={
            "type":"update",
        ...
        var sendScatterAll={
            "type":"update",
            "tablename":"DICT_REC_INFO",
            "set":[["SCATTER_STATUS=","0"]],
            "where":[["SCATTER_STATUS=","2"]]
        }
        $.ajax({
        type:"POST",
        ...
        jsonp: "jsonpCallback",//服务端用于接收callback调用的function名的参数
            success: function(json){
                ...
                if(result == true){
                    loadingShow('white', 'loadingLong', '全部重试中... 请您稍等，谢谢！');
                    setTimeout(function(){
    ...
    js_flowChart.undelegate("click").delegate("i", "click" ,function(){
        ...
        loadingShow('white', 'loadingLong', '重试中... 请您稍等，谢谢！');
        // 重试状态位
        var js_retryval = $(this).attr("name");
    ...
    // 全部重试
    function retryfn(){
        showPop(".popUpRetry", ".iframe_box");
        $(".retryClose").css("border","none");
    };
    // 保存提示功能
